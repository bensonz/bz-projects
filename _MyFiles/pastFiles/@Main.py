...
class monster(pygame.sprite.Sprite):
    ...
    def getMonsterCondition(self):
        return [self.xPos, self.yPos, self.die]

    def AI(self):
        if self.yPos >= 400:
            if 0 > self.xPos - self.playerPos[0] > -300:
                self.dirFace = True
                self.isRunning = True
            elif 0 <= self.xPos - self.playerPos[0] < 300:
                self.dirFace = False
                self.isRunning = True
            else:
                if self.dirFace and self.xPos < 700:
                    self.isRunning = True
                elif not self.dirFace and self.xPos > 0:
                    self.isRunning = True
                else:
    ...
    def update(self, playerCond):
        ...
        if self.monsterHit < 2:
            ...
            if self.xPos >= 800 - self.rect.width:
                self.xPos = 800- self.rect.width
                self.isRunning = False
            elif self.xPos <= 0:
                self.xPos = 0
                self.isRunning = False
...
class player(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.isRunning = False
        self.gravity = 1
        self.xPos, self.yPos = 0,550
        self.xPos += self.rect.left
        self.yPos += self.rect.top
        self.coolDown = 10
        self.health = 1000
        
        self.weaponPos = (self.xPos+20,self.yPos-20)
    ...
    def getPlayerCondition(self):
        return (self.xPos,self.yPos,self.offGround)
    ...
    def keyRead(self):
        ...
        if keys[K_x] and not self.attacking:
            if self.weaponName == "axe":
                ...
                self.spikyBallPos = (self.xPos, self.yPos)
    ...
    def update(self, monsterTouch, itemSelected, healthRegenCoolDown,spikyTouch):
        ...
        if itemSelected != None:
            ...
            if itemSelected == "axe":
                ...
                self.weaponRect.left = self.xPos
                self.weaponRect.bottom = self.yPos
                if self.dirFace:
                    self.weaponPos = (self.xPos+20,self.yPos-20)
                    angle = -70
                if not self.dirFace:
                    weapon = pygame.transform.flip(weapon,True,False)
                    self.weaponPos = (self.xPos-30,self.yPos-20)
                    angle = 70
                if self.attacking:
                    self.attacking = False
                    self.axeSwung = True
                ...
                weapon = pygame.transform.scale(weapon,(36,26))
                if self.dirFace:
                    self.weaponPos = (self.xPos+5, self.yPos)
                elif not self.dirFace:
                    self.weaponPos = (self.xPos-5, self.yPos)
                    weapon = pygame.transform.flip(weapon,True,False)
                if self.rifleFired:
    ...
    def bulletShot(self):
        b = bullets(self.weaponPos, self.aimPos)
...
class bullets(pygame.sprite.Sprite):
 ...
 #       print "should be the same as bullet BLIT point", (self.rect.centerx, self.rect.centery)
        ...
        self.speed = 10.0
        self.x = vector(self.rect.centerx, self.rect.centery)
        self.y = vector(self.targetx, self.targety)
    ...
    def update(self):
        ...
        radians = math.atan2(self.distance[0],self.distance[1]) #OMG This took me long enough to find out
        dx = math.cos(radians)
        dy = math.sin(radians)
